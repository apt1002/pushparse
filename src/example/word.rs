...
pub trait Push: P<char> + P<Whitespace> + P<Alphanumeric> + P<Symbolic> + keyword::Push {}

impl<I: P<char> + P<Whitespace> + P<Alphanumeric> + P<Symbolic> + keyword::Push> Push for I {}
...
impl<I: Push> Parser<I> {
    ...
    pub fn new(inner: I) -> Self where I: keyword::Push {
        let mut keywords = HashMap::new();
        for index in 0..<<Self as Wrap>::Inner as keyword::Push>::List::NUM_KEYWORDS {
            let name = <<Self as Wrap>::Inner as keyword::Push>::List::name(Keyword(index));
            ...
            if keywords.insert(name, keywords.len()).is_some() {
...
mod tests {
    ...
    impl keyword::List for Buffer {
        ...
        fn name(_: Keyword) -> &'static str { "return" }
    }
    ...
    fn check(input: &str, expected: &[Token]) {
        println!("input = {:}", input);
        let mut parser = escape::Parser::new(span::Parser::new(Parser::new(Buffer::default())));
