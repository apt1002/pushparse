...
//! [`Parser`]s.
//!
//! - [`Parser<B, _>`] - a [`crate::Parser`] implementation that accepts:
//!   - [`char`] (to find opening and closing brackets)
//! - [`Bracket<B>`] - a trait that defines the syntax of bracket token `B` and
...
use crate::{E, Push as P, Parser as _, Flush};
...
///
/// Implies `crate::Push<T>` for every type `T` generated by [`Parser<B>`].
pub trait Bracket<B: Spectator>: P<char> + P<B> {
    ...
    /// The type of a [`crate::Parser`] for the contents of the brackets.
...
#[derive(Debug, Clone)]
pub struct Parser<B: Spectator, I: Bracket<B>> {
...
impl<B: Spectator, I: Bracket<B>> Parser<B, I> {
...
impl<B: Spectator, I: Bracket<B>> crate::Parser for Parser<B, I> {
...
impl<B: Spectator, I: Bracket<B>> P<char> for Parser<B, I> {
    fn push(&mut self, token: char) {
        if token == I::OPEN {
            self.open();
        } else if token == I::CLOSE {
            self.close();
        } else {
...
impl<B: Spectator, I: Bracket<B> + crate::Flush> crate::Flush for Parser<B, I> {
...
impl<
    T: Spectator,
    B: Spectator,
    I: Bracket<B> + Bracket<T>,
> Bracket<T> for Parser<B, I> where
    <I as Bracket<B>>::Parser: P<T>,
    <I as Bracket<T>>::Parser: Bracket<B, Parser=<I as Bracket<B>>::Parser>,
{
    const OPEN: char = <I as Bracket<T>>::OPEN;
    const CLOSE: char = <I as Bracket<T>>::CLOSE;
    type Parser = Parser<B, <I as Bracket<T>>::Parser>;
    fn new_parser(&self) -> Self::Parser { Parser::new(<I as Bracket<T>>::new_parser(&self.top_inner)) }
}
...
mod tests {
    ...
    impl Bracket<Box<[Token]>> for Buffer {
        const OPEN: char = '(';
        const CLOSE: char = ')';
        type Parser = Self;
